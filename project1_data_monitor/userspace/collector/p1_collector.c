...
static void config_defaults(struct p1_config *cfg)
{
	snprintf(cfg->server_ip, sizeof(cfg->server_ip), "%s",
		 P1_DEFAULT_SERVER_IP);
	cfg->server_port = P1_DEFAULT_SERVER_PORT;
...
static int parse_args(int argc, char **argv, struct p1_config *cfg)
{
	int i;
...
static int read_ap3216c(struct p1_ap3216c_sample *sample)
{
	return read_full_device(P1_DEV_AP3216C, sample, sizeof(*sample));
}

static int read_icm20608(struct p1_icm20608_sample *sample)
{
	return read_full_device(P1_DEV_ICM20608, sample, sizeof(*sample));
}
...
{
	if (write_output(P1_DEV_LED, on) == 0) {
...
{
	if (write_output(P1_DEV_BEEP, on) == 0) {
		pthread_mutex_lock(&g_rt.lock);
		g_rt.beep_on = on;
		pthread_mutex_unlock(&g_rt.lock);
	}
...
static void apply_alarm_policy(const struct p1_sample_record *rec)
{
	int alarm = 0;
	enum p1_work_mode mode;
	int ps_threshold;
...
static void send_register_message(void)
{
	...
	snprintf(line, sizeof(line),
		 "{\"type\":\"register\",\"device\":\"imx6ull-project1\","
		 "\"version\":\"1.0\"}\n");
	send_line(line);
}

static void send_status_message(const struct p1_sample_record *rec)
{
	char line[P1_LINE_SIZE];
	enum p1_work_mode mode;
	int interval;
...
static void cycle_mode(void)
{
	...
	pthread_mutex_lock(&g_rt.lock);
	mode = g_rt.mode;
	if (mode == P1_MODE_NORMAL)
		g_rt.mode = P1_MODE_QUIET;
	else if (mode == P1_MODE_QUIET)
		g_rt.mode = P1_MODE_ALARM_ONLY;
	else
		g_rt.mode = P1_MODE_NORMAL;
	mode = g_rt.mode;
	pthread_mutex_unlock(&g_rt.lock);

	log_msg("mode changed to %s", p1_mode_name(mode));
}
...
static void *key_thread(void *arg)
{
	...
	if (fd < 0) {
		log_msg("key disabled: open %s failed: %s", P1_DEV_KEY,
			strerror(errno));
		return NULL;
	}
...
static void collect_once(struct p1_sample_record *rec)
{
